...
- **Assunto no TEC.** Cada item mostra a linha `TEC: <matéria> › <código> <assunto> · <n> questões` e um chip **TEC · <código> <assunto>** que abre a página pública do assunto. Vem de [[TEC - Árvore de assuntos]] (gerada por `PY/tec-arvore.py`), casando o tópico da grade e o heading com o nome do assunto por palavras raras (números só ajustam: CPC 02 não casa com CPC 18). Sem casamento confiável o item não mostra a linha e cai no texto "filtre por". Ao criar o caderno, marque esse nó em Filtros → Assunto.
