...
- **Heading de dia:** logo antes da primeira entrada de cada dia (comparando a data `dd/mm` do título com a da última entrada já na nota), insira `### dd/mm` (uma linha, sem hora) com linha em branco antes e depois. Se o dia já tem heading (você está acrescentando mais uma entrada do mesmo dia), não repita — só cai a entrada nova depois da última do dia. Isso deixa a nota navegável pelo esquema do Obsidian e permite pular direto pro dia de hoje sem rolar a nota inteira.
